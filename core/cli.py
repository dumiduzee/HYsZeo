...
@click.option('--action', '-a', required=True, help='Action to perform: start or stop', type=click.Choice(['start', 'stop'], case_sensitive=False))
@click.option('--domain', '-d', required=False, help='Domain name for SSL', type=str)
@click.option('--port', '-p', required=False, help='Port number for ZoraCloud Zero Panel service', type=int)
@click.option('--admin-username', '-au', required=False, help='Admin username for ZoraCloud Zero Panel', type=str)
@click.option('--admin-password', '-ap', required=False, help='Admin password for ZoraCloud Zero Panel', type=str)
@click.option('--expiration-minutes', '-e', required=False, help='Expiration minutes for ZoraCloud Zero Panel session', type=int, default=20)
@click.option('--debug', '-g', is_flag=True, help='Enable debug mode for ZoraCloud Zero Panel', default=False)
...
def webpanel(action: str, domain: str, port: int, admin_username: str, admin_password: str, expiration_minutes: int, debug: bool, decoy_path: str | None):
    """Manages the ZoraCloud Zero Panel service."""
    try:
        if action == 'start':
            ...
            if not services_status:
                raise click.Abort('Error: ZoraCloud Zero Panel services status not available.')
            ...
            click.echo(f'ZoraCloud Zero Panel is now running. The service is accessible on: {url}')
            if decoy_path:
            ...
            click.echo(f'ZoraCloud Zero Panel stopped successfully.')
    except Exception as e:
        click.echo(f'{e}', err=True)

@cli.command('setup-webpanel-decoy')
@click.option('--domain', '-d', required=True, help='Domain name associated with the ZoraCloud Zero Panel', type=str)
...
def setup_webpanel_decoy(domain: str, decoy_path: str):
    """Sets up or updates the decoy site for the running ZoraCloud Zero Panel."""
    try:
        cli_api.setup_webpanel_decoy(domain, decoy_path)
        click.echo(f'ZoraCloud Zero Panel decoy site configured successfully for domain {domain} using path {decoy_path}.')
...
def stop_webpanel_decoy():
    """Stops the decoy site functionality for the ZoraCloud Zero Panel."""
    try:
