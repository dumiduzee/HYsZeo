disable_systemd_resolved() {
    ...
    if systemctl is-active --quiet systemd-resolved; then
        ...
        log_info "systemd-resolved is already stopped."
    fi
    if [ -L /etc/resolv.conf ]; then
        rm -f /etc/resolv.conf
        echo "nameserver 8.8.8.8" > /etc/resolv.conf
        log_success "/etc/resolv.conf updated to use 8.8.8.8."
    fi
}
#!/bin/bash
...
clone_repository() {
    log_info "Cloning ZoraCloud Zero repository..."
    ...
    if git clone https://github.com/dumiduzee/Hys-Zero-Pannel.git /etc/hysteria &> /dev/null; then
...
run_menu() {
    ...
    echo -e "\n${BOLD}${GREEN}======== Launching ZoraCloud Zero Menu ========${NC}\n"
    ./menu.sh
}

main() {
    echo -e "\n${BOLD}${BLUE}======== ZoraCloud Zero Setup Script ========${NC}\n"
    ...
    echo -e "\n${YELLOW}Starting ZoraCloud Zero in 3 seconds...${NC}"
    sleep 3
